...
#include "agiros/bridge/rotors_bridge.hpp"
...
RosPilot::RosPilot(const ros::NodeHandle& nh, const ros::NodeHandle& pnh)
  ...
  guard_pose_estimate_sub_ = pnh_.subscribe(
    "guard_pose_estimate", 1, &RosPilot::guardPoseEstimateCallback, this,
    ros::TransportHints().tcpNoDelay());
  guard_odometry_estimate_sub_ = pnh_.subscribe(
    "guard_odometry_estimate", 1, &RosPilot::guardOdometryEstimateCallback,
    this, ros::TransportHints().tcpNoDelay());
  guard_state_estimate_sub_ = pnh_.subscribe(
    "guard_state_estimate", 1, &RosPilot::guardStateEstimateCallback, this,
    ros::TransportHints().tcpNoDelay());
  // Logic subscribers
  ...
  guard_sub_ =
    pnh_.subscribe("trigger_guard", 1, &RosPilot::triggerGuardCallback, this);
...
void RosPilot::stateEstimateCallback(const agiros_msgs::QuadState& msg) {
  QuadState state;
  state.setZero();
...
void RosPilot::guardPoseEstimateCallback(
  const geometry_msgs::PoseStampedConstPtr& msg) {
  Pose pose;
  pose.t = msg->header.stamp.toSec();
  pose.position = fromRosVec3(msg->pose.position);
...
void RosPilot::guardStateEstimateCallback(const agiros_msgs::QuadState& msg) {
  QuadState state;
  state.setZero();
  state.t = msg.header.stamp.toSec();
  state.p = fromRosVec3(msg.pose.position);
...
void RosPilot::guardOdometryEstimateCallback(
...
void RosPilot::triggerGuardCallback(const std_msgs::EmptyConstPtr& msg) {
